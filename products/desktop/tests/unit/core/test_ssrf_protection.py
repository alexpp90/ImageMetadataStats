...
def temp_config_dir(tmp_path):
    ...
    import photo_selector_toolbox.core.cache as cache
...
@patch("urllib.request.OpenerDirector.open")
def test_ollama_tool_blocks_unspecified_ip_ipv4_mapped(mock_open, dummy_image_file, temp_config_dir):
    from photo_selector_toolbox.core.config import save_config
    save_config({
        "ollama_url": "http://[::ffff:0.0.0.0]/latest/meta-data/",
        "ollama_model": "test",
        "ollama_prompt": "test"
    })
    tool = OllamaAestheticTool()
...
@patch("urllib.request.OpenerDirector.open")
def test_ollama_tool_blocks_unspecified_ip(mock_open, dummy_image_file, temp_config_dir):
    from photo_selector_toolbox.core.config import save_config
    save_config({
        "ollama_url": "http://0.0.0.0/latest/meta-data/",
        "ollama_model": "test",
        "ollama_prompt": "test"
    })
    tool = OllamaAestheticTool()
