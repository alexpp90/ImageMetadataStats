...
def _calculate_noise_from_gray(gray: np.ndarray) -> float:
    ...
    laplacian = cv2.Laplacian(gray, cv2.CV_32F)  # OPTIMIZATION: CV_32F is faster than CV_64F
...
def _calculate_sharpness_from_gray(gray: np.ndarray, grid_size: int = 1) -> float:
    ...
    if grid_size <= 1:
        return float(cv2.Laplacian(cropped, cv2.CV_32F).var())  # OPTIMIZATION: CV_32F is faster than CV_64F
    ...
    for r in range(grid_size):
        for c in range(grid_size):
            ...
            score = cv2.Laplacian(block, cv2.CV_32F).var()  # OPTIMIZATION: CV_32F is faster than CV_64F
            if score > max_score:
                max_score = score
