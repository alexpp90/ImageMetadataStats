...
def _calculate_noise_from_gray(gray: np.ndarray) -> float:
    ...
    # Bolt optimization: Using CV_32F is significantly faster and sufficient for Laplacian noise detection
...
def _calculate_sharpness_from_gray(gray: np.ndarray, grid_size: int = 1) -> float:
    ...
    if grid_size <= 1:
        # Bolt optimization: CV_32F speeds up Laplacian var by ~3x
